    ...
    <div className="fixed bottom-6 right-6 z-[999] font-sans">
      ...
      {!isOpen && (
        <button
          ...
          className="w-16 h-16 bg-[#00e5ff] rounded-full flex items-center justify-center text-black shadow-[0_0_20px_rgba(0,229,255,0.4)] hover:scale-110 transition-all group"
        >
      ...
      {isOpen && (
        <div className="w-[350px] sm:w-[400px] h-[500px] bg-[#09090b] border border-white/10 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-10">
          {/* Header */}
          <div className="bg-[#00e5ff] p-4 flex justify-between items-center">
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.isBot ? "justify-start" : "justify-end"}`}>
                <div className={`max-w-[80%] p-3 rounded-2xl text-sm font-medium ${
                  msg.isBot ? "bg-white/5 text-white border border-white/10" : "bg-[#00e5ff] text-black font-bold"
                }`}>
                  {msg.text}
                </div>
              </div>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-white/5 p-3 rounded-2xl flex gap-1">
                  ...
                  <div className="w-1.5 h-1.5 bg-[#00e5ff] rounded-full animate-bounce delay-75"></div>
          ...
          <div className="p-4 border-t border-white/5 bg-[#050814]">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => handleAction("siparis")} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-[10px] font-bold text-slate-300 hover:border-[#00e5ff] hover:text-[#00e5ff] transition-all flex items-center gap-2">
                <MapPin size={12} /> SİPARİŞİM NEREDE?
              </button>
              <button onClick={() => handleAction("kargo")} className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-[10px] font-bold text-slate-300 hover:border-[#00e5ff] hover:text-[#00e5ff] transition-all flex items-center gap-2">
                <Info size={12} /> KARGO BİLGİSİ
              </button>
              <button onClick={() => handleAction("canli")} className="px-3 py-1.5 bg-[#00e5ff]/10 border border-[#00e5ff]/20 rounded-lg text-[10px] font-black text-[#00e5ff] hover:bg-[#00e5ff] hover:text-black transition-all flex items-center gap-2 w-full justify-center">
