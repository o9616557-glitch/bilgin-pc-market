...
// EĞİTİM NOTU: 7 ANA KATEGORİ VE ALT KATEGORİLERİMİZ
const navigation = [
  ...
  return (
    <header className="w-full bg-[#050810] border-b border-white/5 sticky top-0 z-[100]">
      ...
      <div className="max-w-[1400px] mx-auto px-5 h-20 flex items-center justify-between">
        ...
        {/* ORTA BÖLÜM: İNCE YAZILI RAZER STİLİ KATEGORİLER VE TİTREMEYEN MEGA MENÜ */}
            ...
            <div key={item.name} className="group h-full flex items-center px-3 xl:px-4 cursor-pointer">
              ...
              <div className="absolute top-full left-0 w-full bg-[#0b0f1a] border-t border-b border-white/5 hidden group-hover:block z-[110] shadow-[0_20px_50px_rgba(0,0,0,0.7)]">
                <div className="max-w-[1400px] mx-auto px-5 py-10 flex gap-12">
                  ...
                  {/* Sağ Taraf: Yardım ve Keşif (Gerçek Linkler) */}
                  <div className="w-2/3 flex flex-col gap-4">
                    <h3 className="text-[10px] text-slate-500 font-black uppercase tracking-widest mb-2 border-b border-white/5 pb-2">Yardım ve Keşif</h3>
                    <Link href={`/kategori`} className="text-slate-400 hover:text-white text-xs font-medium transition-colors">Tüm {item.name} Ürünlerini Gör</Link>
                    <Link href="/yeni-gelenler" className="text-slate-400 hover:text-white text-xs font-medium transition-colors">Yeni Gelen Modeller</Link>
                    <Link href="/sss" className="text-slate-400 hover:text-white text-xs font-medium transition-colors">Sıkça Sorulan Sorular (S.S.S)</Link>
                    <Link href="/iletisim" className="text-slate-400 hover:text-white text-xs font-medium transition-colors">Destek ve İletişim</Link>
                  </div>
        ...
        {/* SAĞ BÖLÜM: İKONLAR (Arama, Hesabım, Sepet) */}
        <div className="flex items-center gap-6 xl:gap-8 h-full">
           ...
           <div className="relative flex items-center h-full group/account cursor-pointer">
             ...
             {/* EĞİTİM NOTU: Kalp (svg) ikonunu buradan sildik. Favorilerim sade metin olarak kaldı. */}
