...
export default function Header() {
  ...
  const [isAccountOpen, setIsAccountOpen] = useState(false); // Hesabım Popup Hafızası

  return (
    // EĞİTİM NOTU: Tüm menüyü tutan ana çerçeve. 'h-20' ile yüksekliği sabitledik.
    <header className="w-full bg-[#050810] border-b border-white/5 sticky top-0 z-[100]">
      
      {/* TEK SIRA (SINGLE ROW) DÜZENİ */}
      <div className="max-w-[1400px] mx-auto px-5 h-20 flex items-center justify-between">
        ...
        {/* ORTA BÖLÜM: 7 KATEGORİ (Sadece PC'de görünür - lg:flex) */}
        <nav className="hidden lg:flex items-center h-full">
          {navigation.map((item) => (
            // group div'i göreceli YAPILMADI (relative YOK). Bu sayede menü ekranı boydan boya kaplayabilir.
            <div key={item.name} className="group h-full flex items-center px-2 xl:px-3 cursor-pointer">
              
              <Link href="#" className="text-[9px] xl:text-[10px] font-black capitalize tracking-widest text-slate-400 group-hover:text-white transition-colors whitespace-nowrap">
                {item.name}
              </Link>

              {/* TİTREMEYEN MEGA MENÜ KUTUSU */}
              {/* EĞİTİM NOTU: Animasyonlar silindi, sadece 'hidden group-hover:block' kullanıldı. Böylece fare yan kategoriye geçince eskisi şıp diye kapanıp yenisi şıp diye açılır, titreme olmaz. */}
              <div className="absolute top-full left-0 w-full bg-[#0b0f1a] border-t border-b border-white/5 hidden group-hover:block z-[110] shadow-[0_20px_50px_rgba(0,0,0,0.7)]">
                <div className="max-w-[1400px] mx-auto px-5 py-10 flex gap-12">
                  ...
                  <div className="w-px bg-white/5"></div> {/* İnce Ayırıcı Çizgi */}

                  {/* SAĞ KOLON: Ekstra Bağlantılar */}
        ...
        {/* SAĞ BÖLÜM: İKONLAR (Arama, Favoriler, Hesabım Popup, Sepet) */}
        <div className="flex items-center gap-4 xl:gap-6">
           ...
           <Link href="/favoriler" className="text-slate-400 hover:text-red-500 transition-colors">
             <svg className="w-5 h-5 xl:w-6 xl:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
           </Link>
           
           {/* HESABIM POPUP */}
           <div className="relative flex items-center">
             <button onClick={() => setIsAccountOpen(!isAccountOpen)} className={`text-slate-400 hover:text-white transition-colors p-1 rounded-full ${isAccountOpen ? 'bg-white/10 text-white' : ''}`}>
             ...
             {/* Tıklanınca alt alta açılan sipariş, hesabım, giriş menüsü */}
             {isAccountOpen && (
               <div className="absolute top-[150%] right-0 w-48 bg-[#0b0f1a] border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.5)] rounded-2xl flex flex-col z-[120] overflow-hidden animate-in fade-in slide-in-from-top-2">
