...
export default function Header() {
  ...
  // 🚀 ŞEFİN SİNYAL DİNLEME MOTORU: Sepet anında güncellenir!
  useEffect(() => {
    const updateCount = () => {
      // Ürün sayfasının veriyi kaydettiği yer "cart"
      const storedCart = localStorage.getItem("cart");
      if (storedCart) {
        try {
          const cart = JSON.parse(storedCart);
          // Sepetteki toplam ürün adetini hesapla
      ...
      <div key={product.id} className="bg-white/5 border border-white/5 hover:border-white/10 p-5 rounded-3xl group/card transition-all duration-300 flex flex-col justify-between space-y-4 shadow-xl">
        <div className="space-y-4">
          ...
          <div className="flex flex-col space-y-1">
            ...
            <h4 className="text-sm font-bold italic tracking-tighter text-white leading-tight group-hover/card:text-blue-500 transition-colors line-clamp-2">
              {product.name}
            </h4>
          </div>
        </div>
        <div className="flex items-center justify-between gap-3 pt-3 border-t border-white/5 mt-auto">
          ...
          <Link href={`/product/${product.slug}`} onClick={() => setIsSearchOpen(false)} className="w-32 py-3 bg-green-500 text-black font-black uppercase rounded-xl hover:scale-105 transition-transform text-[11px] tracking-widest flex items-center justify-center">
            İncele
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-4 xl:px-5 h-20 flex items-center justify-between relative">
        ...
        <div className="flex items-center gap-4 xl:gap-7 flex-shrink-0 relative">
          ...
          >
            ...
            <div className={`absolute top-[80%] right-[-10px] pt-5 transition-all duration-200 z-[120] ${isAccountOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'}`}>
              <div className="absolute top-[12px] right-[18px] w-4 h-4 bg-[#0b0f1a] border-t border-l border-white/10 rotate-45"></div>
              <div className="relative z-10 w-48 bg-[#0b0f1a] border border-white/10 shadow-2xl rounded-2xl flex flex-col overflow-hidden">
                <Link href="/siparis-takip" className="px-5 py-3.5 text-xs font-medium text-slate-300 hover:bg-white/5 hover:text-white border-b border-white/5">Sipariş Takip</Link>
          ...
          <Link href="/sepet" className="text-slate-400 hover:text-green-500 relative p-1 transition-colors">
            <svg className="w-5 h-5 xl:w-6 xl:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 010-4zm-8 0a2 2 0 100 4 2 2 0 010-4z" /></svg>
      ...
      {isSearchOpen && (
        <div ref={searchOverlayRef} className="fixed inset-0 bg-[#050810]/95 z-[200] overflow-y-auto backdrop-blur-md animate-in fade-in">
          ...
          <div className="sticky top-0 w-full h-20 px-5 border-b border-white/5 flex items-center justify-between z-10 bg-[#050810]/80">
            <Link href="/" onClick={() => setIsSearchOpen(false)} className="text-xl font-black italic tracking-tighter text-white">
              ...
              <div className="space-y-4">
                ...
                <div className="flex flex-wrap gap-2.5">
                  {quickSearches.map(term => (
                    <button key={term} onClick={() => setSearchTerm(term)} className="px-4 py-2 bg-white/5 border border-white/5 rounded-xl text-slate-300 hover:text-white hover:border-white/10 hover:bg-white/10 text-xs font-medium transition-all">
                      {term}
                    </button>
                  ))}
                </div>
            ...
            <main className="w-full md:w-3/4 space-y-12">
              <div className="relative max-w-3xl">
                <input 
                  ...
                  className="w-full bg-[#0b1120] rounded-2xl text-2xl md:text-3xl font-black italic text-white placeholder-slate-600 focus:outline-none focus:ring-4 focus:ring-blue-500/20 border border-white/5 transition-all p-6 pl-20"
                />
