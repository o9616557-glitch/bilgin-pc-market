...
/* ─────────────────── MOBİL HESAP MENÜ ÇUBUĞU (hamburger → tam ekran sayfa) ─────────────────── */
function MobilHesapMenuLink({ active }: { active?: string }) {
  ...
  return (
    <div className="account-card p-2 flex items-center gap-1">
      <div className="flex items-center gap-2.5 flex-1 min-w-0 px-3 py-2.5">
        <AktifIcon className="w-4 h-4 shrink-0 text-site-accent" />
        <span className="text-sm font-medium text-slate-200 truncate">{aktifItem.label}</span>
      </div>
      <button
        type="button"
        onClick={() => router.push("/hesap-menu")}
        className="flex items-center justify-center w-10 h-10 rounded-xl hover:bg-white/[0.05] transition-colors shrink-0 active:scale-95"
