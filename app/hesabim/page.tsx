...
export default function HesabimPage() {
  ...
  // 🚀 ADIM 1: Ana iş parçacığını (Main Thread) kilitlememek için başlangıç değerlerini boş bırakıyoruz
  ...
  const [loading, setLoading] = useState(true); // Yüklenme durumunu yöneteceğiz
  ...
  // 🚀 ADIM 2: Sayfa "çizildikten hemen sonra" hafızayı okuyoruz, böylece ekran donmuyor!
  useEffect(() => {
    ...
    try {
      ...
      if (hafiza) {
        ...
        if (parsed.tumSiparisler) setHamSiparisler(parsed.tumSiparisler);
      ...
      const destekHafiza = sessionStorage.getItem("bilgin_destek_ozet");
      if (destekHafiza) {
        const parsed = JSON.parse(destekHafiza);
        if (parsed.sayi !== undefined) setAcikTalepSayisi(parsed.sayi);
        if (parsed.acil !== undefined) setYeniMesajVar(parsed.acil);
      }
      
      setLoading(false); // Hafızayı okuduk, ekranı göster!
    } catch (error) {
  ...
  // 🚀 ADIM 3: SESSİZ ÇIRAK (Ekranı dondurmadan verileri günceller)
  useEffect(() => {
  ...
  // GRAFİK VE MATEMATİK HESAPLAMALARI
  useEffect(() => {
  ...
  // Yükleme ekranı çok kısa sürecek, sadece veriler hafızadan çekilene kadar gösterilir
  if (loading) {
    return (
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans p-4 sm:p-6 lg:p-8 relative overflow-clip animate-in fade-in duration-300">
      ...
      <div className="max-w-[1400px] mx-auto flex flex-col lg:flex-row gap-5 lg:gap-8 relative z-10 items-start">
        ...
        {/* ⬅️ SOL MENÜ (prefetch={false} tamamen kaldırıldı, Roket Motoru devrede!) */}
        ...
        <div className="flex-1 flex flex-col min-w-0 gap-5 lg:gap-6 w-full">
          ...
          <div className="grid grid-cols-3 lg:grid-cols-6 gap-x-2 gap-y-4 sm:gap-4 w-full">
             <Link href="/adreslerim" onClick={kilitliIslem} className="flex flex-col gap-1.5 group">
             ...
             <Link href="/siparis-takip" onClick={kilitliIslem} className="flex flex-col gap-1.5 group">
             ...
             <Link href="/favorilerim" onClick={kilitliIslem} className="flex flex-col gap-1.5 group">
             ...
             <Link href="/sistemlerim" onClick={kilitliIslem} className="flex flex-col gap-1.5 group">
            ...
            <Link href="/destek-taleplerim" onClick={kilitliIslem} className="flex flex-col gap-1.5 group relative">
               {yeniMesajVar && (
            ...
            <div className="xl:col-span-1 flex flex-col h-full">
              <div className="bg-[#0f172a] border border-slate-800 rounded-2xl p-6 shadow-xl relative overflow-hidden group hover:border-cyan-500/30 transition-all duration-300 flex flex-col h-[350px] sm:h-[450px] xl:h-[550px]">
               ...
               <div className="flex items-center justify-between mb-4 sm:mb-6 pb-3 sm:pb-4 border-b border-slate-800/80 relative z-10 shrink-0">
                  ...
                  <Link href="/siparislerim" onClick={kilitliIslem} className="text-[10px] sm:text-xs font-bold text-cyan-400 hover:underline tracking-widest uppercase">
                    TÜMÜNÜ GÖR
                  </Link>
                </div>
