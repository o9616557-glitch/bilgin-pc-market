    ...
    <div className="min-h-screen bg-[#050814] text-white pt-12 pb-24 px-4 relative overflow-hidden font-sans">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 border-b border-slate-800 pb-6 mb-10">
          ...
          {/* Sipariş Sayısı Bölümü Hayaleti */}
          <div className="w-36 h-11 bg-slate-800/40 rounded-xl animate-pulse shadow-lg"></div>
        </div>

        {/* 🚀 SİPARİŞ KARTI HAYALETİ */}
        <div className="flex flex-col gap-6">
          <div className="border border-slate-800 bg-[#09090b]/50 rounded-2xl overflow-hidden shadow-xl">
            
            {/* Sipariş Üst Bar Hayaleti (Tarih, No, Tutar) */}
            <div className="bg-[#121215]/40 border-b border-slate-800/60 p-4 flex flex-wrap items-center justify-between gap-4">
              <div className="flex items-center gap-6">
                <div className="h-4 bg-slate-800/40 rounded w-24 animate-pulse"></div>
                <div className="h-4 bg-slate-800/40 rounded w-32 animate-pulse"></div>
              </div>
              <div className="h-5 bg-slate-800/40 rounded w-28 animate-pulse"></div>
            </div>

            {/* Sipariş İçerik Alanı Hayaleti */}
            <div className="p-5 flex flex-col sm:flex-row sm:items-center gap-5">
              <div className="w-20 h-20 bg-slate-800/40 rounded-xl animate-pulse shrink-0"></div>
              <div className="flex-1">
                <div className="h-4 bg-slate-800/40 rounded w-2/3 mb-3 animate-pulse"></div>
                <div className="h-4 bg-slate-800/40 rounded w-1/4 animate-pulse"></div>
              </div>
              <div className="w-24 h-9 bg-slate-800/40 rounded-lg animate-pulse"></div>
            </div>
