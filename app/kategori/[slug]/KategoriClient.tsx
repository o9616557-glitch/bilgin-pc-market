...
export default function KategoriClient({ urunler, sayfaBasligi }: { urunler: any[], sayfaBasligi: string }) {
  ...
  return (
    <>
      ...
      {/* 🛠️ SOL FİLTRE MENÜSÜ (Header'ın altına giren KALIN Buzlu Cam Versiyon) */}
        <aside className={`fixed top-[100px] bottom-0 left-0 right-0 z-[40] lg:sticky lg:top-24 lg:w-[260px] xl:w-[280px] lg:max-h-[calc(100vh-100px)] lg:shrink-0 transition-transform duration-300 flex flex-col ${mobilFiltreAcik ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
          <div className="absolute inset-0 bg-[#050814]/90 backdrop-blur-2xl lg:hidden" onClick={() => setMobilFiltreAcik(false)}></div>
