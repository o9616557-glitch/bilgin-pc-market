...
import { MapPin, Plus, Trash2, ArrowLeft, Mail, Phone } from "lucide-react";
...
export default function AdresYoneticisi({ initialAddresses }: Props) {
  ...
  const handleAddAddress = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        // 🚀 BİNGO: Ters tırnak hatası artı (+) işareti ile çözüldü!
      ...
      {showForm && (
        <div className="bg-[#080d1a] border border-white/10 rounded-2xl p-6 mb-8 animate-in fade-in slide-in-from-top-4">
         <div className="mb-6 border-b border-white/10 pb-4 flex justify-between items-center">
            ...
            <button onClick={() => setShowForm(false)} className="text-slate-500 hover:text-white text-xs font-bold uppercase">Kapat</button>
          </div>
          ...
          <form onSubmit={handleAddAddress} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input type="email" name="email" value={formData.email} onChange={handleInputChange} required className="bg-[#050B14] border border-white/10 rounded-lg p-3 text-sm focus:border-[#3b82f6]/50 outline-none transition-colors" />
            </div>
