...
// EĞİTİM: Bu ayar kodun direkt sunucuda çalışmasını sağlar (Sıfır Bekleme)
...
export default async function UrunDetay({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Hızlı veri çekimi (Müşteri beklemez)
  ...
  return (
    <div className="bg-[#050810] min-h-screen pb-32 text-white font-sans selection:bg-blue-500/30">
      ...
      <div className="max-w-5xl mx-auto px-5 space-y-8">
        ...
        {/* ========================================================= */}
        {/* TEKNİK BİLGİLER (Artık Belirgin Bir Ana Kutu İçinde) */}
        {/* ========================================================= */}
        <div className="bg-[#0b0f1a] rounded-[2rem] border border-slate-800/60 p-6 md:p-8 shadow-xl">
           <h3 className="text-white font-bold text-sm md:text-base uppercase tracking-wider mb-6 flex items-center gap-2">
             <span className="text-blue-500 text-xl">⚙️</span> Teknik Özellikler
           </h3>
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(acf).map(([key, value]: any) => {
              if (!value || typeof value === 'object' || key.toLowerCase().includes('fps')) return null;
              return (
                <div key={key} className="bg-[#111827] p-4 rounded-xl border border-slate-700/50 flex flex-col justify-center text-center">
        ...
        <details className="group bg-[#0b0f1a] rounded-[2rem] border border-slate-800/60 overflow-hidden" open>
          <summary className="p-6 md:p-8 cursor-pointer list-none flex justify-between items-center select-none">
            <span className="text-white font-bold uppercase tracking-wider text-sm md:text-base flex items-center gap-2">
              <span className="text-blue-500 text-xl">📄</span> Detaylı İnceleme
            </span>
            <span className="text-blue-500 group-open:rotate-180 transition-transform">▼</span>
          </summary>
          <div 
            className="p-6 md:p-8 pt-0 text-sm md:text-base text-slate-400 leading-relaxed border-t border-slate-800/60 mt-4 [&>p]:mb-4 [&>h1]:text-xl [&>h1]:font-black [&>h1]:text-white" 
            dangerouslySetInnerHTML={{ __html: uzunAciklama }} 
          />
        </details>

        {/* ========================================================= */}
        {/* OYUN PERFORMANSI (Artık Kapalı Geliyor. Tıklayınca Açılır)*/}
        {/* open etiketi silinerek kapalı olması sağlandı.            */}
        {/* ========================================================= */}
        <details className="group bg-[#0b0f1a] rounded-[2rem] border border-slate-800/60 overflow-hidden">
          <summary className="p-6 md:p-8 cursor-pointer list-none flex justify-between items-center select-none">
            <span className="text-white font-bold uppercase tracking-wider text-sm md:text-base flex items-center gap-2">
          ...
          <div className="p-6 md:p-8 pt-0 border-t border-slate-800/60 mt-4">
      ...
      {/* YAPIŞKAN SEPET VE FİYAT ÇUBUĞU */}
