...
export default function ProductDetailPage() {
    ...
    const fetchProductData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          console.error("Ürün bulunamadı şefim");
        }
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="min-h-[calc(100vh-80px)] bg-[#050810] flex items-center justify-center">
        <div className="w-10 h-10 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
        <span className="text-white ml-3 font-medium">Ürün Detayları Yükleniyor...</span>
      </div>
    );
  }
