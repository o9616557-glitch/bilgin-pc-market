...
export default function ProductReviews({ productId }: { productId: number }) {
  ...
  const send = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccess("Yorumunuz başarıyla iletildi. Onaylanınca görünecektir.");
  ...
  return (
    <div className="space-y-6 pt-3">
      {!showForm ? (
        ...
        <form onSubmit={send} className="p-4 sm:p-5 rounded-xl bg-[#0b1329] border border-blue-500/30 flex flex-col gap-4 shadow-[0_0_20px_rgba(37,99,235,0.05)]">
          <div className="flex items-center justify-between border-b border-white/5 pb-3"><h3 className="text-blue-400 font-black text-xs uppercase tracking-widest">Deneyiminizi Yazın</h3><button type="button" onClick={() => setShowForm(false)} className="text-slate-500 hover:text-white transition-colors">✕</button></div>
          ...
          <div className="flex flex-col gap-1.5"><label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Puan Verin</label><div className="flex items-center gap-1 cursor-pointer">{[1, 2, 3, 4, 5].map((star) => (<button key={star} type="button" onClick={() => setNewReview({ ...newReview, rating: star })} className={`text-2xl transition-all hover:scale-110 ${star <= newReview.rating ? 'text-amber-400 drop-shadow-[0_0_5px_rgba(251,191,36,0.5)]' : 'text-slate-600 hover:text-amber-400/50'}`}>★</button>))}</div></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><input required type="text" value={newReview.reviewer} onChange={(e) => setNewReview({ ...newReview, reviewer: e.target.value })} className="bg-[#050814]/50 border border-white/10 text-slate-200 rounded-lg p-3 text-sm focus:outline-none focus:border-blue-500/50 transition-colors" placeholder="Adınız Soyadınız" /><input required type="email" value={newReview.email} onChange={(e) => setNewReview({ ...newReview, email: e.target.value })} className="bg-[#050814]/50 border border-white/10 text-slate-200 rounded-lg p-3 text-sm focus:outline-none focus:border-blue-500/50 transition-colors" placeholder="E-posta Adresiniz" /></div>
          <textarea required value={newReview.review} onChange={(e) => setNewReview({ ...newReview, review: e.target.value })} rows={4} className="w-full bg-[#050814]/50 border border-white/10 text-slate-200 rounded-lg p-3 text-sm focus:outline-none focus:border-blue-500/50 resize-none transition-colors" placeholder="Ürün yorumunuzu detaylıca buraya yazın..." />
          <button type="submit" disabled={submitting} className="bg-blue-600 hover:bg-blue-500 text-white font-black px-8 py-3 rounded-lg text-xs uppercase tracking-widest sm:self-end transition-all shadow-[0_0_15px_rgba(37,99,235,0.3)] disabled:opacity-50 disabled:cursor-not-allowed">{submitting ? "Gönderiliyor..." : "Yorumu Gönder"}</button>
        </form>
      )}
      ...
      <div className="space-y-4 mt-4">
        {reviews.map((r) => {
          ...
          return (
            <div key={r.id} className="p-4 sm:p-5 rounded-xl bg-[#050814]/40 border border-white/5 space-y-3 sm:space-y-4 hover:border-white/10 transition-colors">
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col"><span className="text-xs sm:text-sm font-black text-slate-200">{r.reviewer}</span><span className="text-[9px] sm:text-[10px] text-slate-500">{new Date(r.date_created).toLocaleDateString('tr-TR', { year: 'numeric', month: 'long', day: 'numeric' })}</span></div>
                </div>
              ...
              {rReplies.map((rep) => (
                <div key={rep.id} className="bg-blue-600/10 border border-blue-500/20 p-3 sm:p-4 rounded-lg ml-0 sm:ml-11 mt-3 sm:mt-4 text-xs sm:text-sm relative overflow-hidden">
                  ...
                  <div className="text-[10px] sm:text-xs text-blue-400 font-black uppercase mb-1.5 flex items-center gap-1.5"><span className="text-sm">👨‍💻</span> Mağaza Yetkilisi Yanıtı</div>
