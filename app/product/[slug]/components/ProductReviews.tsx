    ...
    <div className="space-y-6 pt-3">
      {!showForm ? (
        ...
        <form onSubmit={send} className="p-5 rounded-xl bg-[#0b1329] border border-blue-500/30 flex flex-col gap-4 shadow-[0_0_20px_rgba(37,99,235,0.05)]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><input required type="text" value={newReview.reviewer} onChange={(e) => setNewReview({ ...newReview, reviewer: e.target.value })} className="bg-[#050814]/50 border border-white/10 text-slate-200 rounded-lg p-3 text-xs focus:outline-none focus:border-blue-500/50 transition-colors" placeholder="Adınız Soyadınız" /><input required type="email" value={newReview.email} onChange={(e) => setNewReview({ ...newReview, email: e.target.value })} className="bg-[#050814]/50 border border-white/10 text-slate-200 rounded-lg p-3 text-xs focus:outline-none focus:border-blue-500/50 transition-colors" placeholder="E-posta Adresiniz" /></div>
