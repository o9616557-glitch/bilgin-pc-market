  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.token) {
        ...
        // WordPress'ten gelen hata mesajını göster (Kullanıcı adı/Şifre yanlışsa)
        setError("Kullanıcı adı veya şifre hatalı. Lütfen kontrol edin.");
      }
    } catch (err) {
      setError("Dükkan sunucusuna bağlanılamadı. Lütfen internetinizi kontrol edin.");
    } finally {
      setIsLoading(false);
    }
  };

  // 2. SOSYAL GİRİŞ (Google & Facebook)
  const handleSocialLogin = (platform: string) => {
    setIsLoading(true);
    // Şefim, WordPress'teki Nextend Social Login linkine ateşliyoruz
    ...
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center p-4 bg-[#050810] relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-10">
          <Link href="/" className="inline-block text-4xl font-black italic tracking-tighter text-white uppercase group transition-all">
        ...
        <div className="bg-[#0b1120] p-8 md:p-10 rounded-3xl border border-white/5 shadow-2xl relative overflow-hidden">
          ...
          <h1 className="text-xl font-black text-white uppercase tracking-widest text-center mb-8">Giriş Yap</h1>

          {error && (
            <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-500 text-[10px] font-bold text-center uppercase tracking-wider animate-bounce">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center px-1">
                ...
                <Link href="/sifre-sifirla" className="text-[9px] font-black text-blue-500 hover:text-blue-400 transition-all uppercase tracking-widest">Şifremi Unuttum?</Link>
              </div>
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-[#050810] border border-white/5 rounded-xl px-5 py-4 text-white font-medium focus:outline-none focus:border-blue-500/50 transition-all placeholder:text-slate-900"
                />
                {/* ŞEFİM: GİRİŞ PANELİNE DE GÖZÜ TAKTIM */}
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-600 hover:text-white text-[10px] font-black uppercase tracking-tighter">
            ...
            <button 
              type="submit" 
              className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black uppercase tracking-widest text-sm py-4 rounded-xl transition-all shadow-[0_0_20px_rgba(37,99,235,0.2)] active:scale-[0.98]"
            >
              Giriş Yap
            </button>
          </form>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
                <button type="button" onClick={() => handleSocialLogin('Google')} className="bg-white/5 border border-white/5 py-3.5 rounded-xl text-[10px] font-black text-slate-300 uppercase hover:bg-white/10 transition-all hover:text-white">Google</button>
                <button type="button" onClick={() => handleSocialLogin('Facebook')} className="bg-blue-600/5 border border-blue-600/20 py-3.5 rounded-xl text-[10px] font-black text-blue-500 uppercase hover:bg-blue-600/10 transition-all hover:text-blue-400">Facebook</button>
            </div>
          ...
          <div className="mt-10 pt-6 border-t border-white/5 text-center">
            <p className="text-[11px] font-medium text-slate-500 uppercase tracking-widest">
              Hesabınız yok mu? 
              <Link href="/kayit" className="text-white font-black hover:text-blue-500 ml-2 transition-colors uppercase">Kayıt Ol</Link>
            </p>
          </div>
        </div>
      </div>
