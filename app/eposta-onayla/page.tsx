    ...
    const verifyEmail = async () => {
      try {
        ...
        try {
          ...
          // JSON parse hatası
          setStatus("error");
          setMessage("Sunucudan geçersiz yanıt alındı.");
          return;
        }
    ...
    <div className="w-full max-w-md bg-[#09090b] border border-white/10 rounded-3xl shadow-[0_0_50px_rgba(0,229,255,0.05)] p-8 relative z-10 text-center">
      ...
      {status === "success" && (
        <div className="flex flex-col items-center gap-4">
          ...
          <Link
            href="/giris"
            className="w-full bg-[#3b82f6] text-black rounded-xl py-3.5 font-black uppercase tracking-widest hover:bg-[#00c4db] transition-all shadow-[0_0_20px_rgba(0,229,255,0.2)]"
          >
            GİRİŞ YAPMAYA GİT
          </Link>
        </div>
      ...
      {status === "error" && (
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-400 text-sm font-medium mb-4">{message}</p>
          {/* Geçici debug — sorunu bulduktan sonra bu satırı sil */}
          {errorDetail && (
            <p className="text-xs text-red-400 bg-red-950/30 rounded p-2 w-full text-left break-all">
              {errorDetail}
            </p>
          )}
          <Link
            href="/kayit"
            className="w-full bg-white/5 border border-white/10 text-white rounded-xl py-3.5 font-black uppercase tracking-widest hover:bg-white/10 transition-all"
          >
            YENİDEN KAYIT OL
          </Link>
        </div>
