...
export default function ForgotPasswordPage() {
  ...
  // Sayfa açılırken veya geri dönülürken yükleme ekranını sıfırlayan sensör
  useEffect(() => {
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    try {
      // 🚀 Canlı API kuryemize e-postayı gönderiyoruz
      ...
      if (res.ok) {
        ...
        // Gelen hatayı küçük harfe uyumlu hale getirip ekrana basıyoruz
        setError(data.error ? data.error.replace(/<[^>]*>?/gm, '') : "Sıfırlama e-postası gönderilemedi.");
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center p-4 bg-[#050810] relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md relative z-10 bg-[#0b1120] p-8 md:p-10 rounded-3xl border border-white/5 shadow-2xl overflow-hidden">
        ...
        {/* DIĞER SAYFALARLA EŞİTLENMİŞ LOGO ALANI */}
        ...
        {/* TATLI TURUNCU VE KÜÇÜK HARFLİ HATA KUTUSU */}
        {error && (
        ...
        {!isSent ? (
          ...
          <div className="text-center py-6 space-y-4 animate-in fade-in duration-300">
            ...
            <p className="text-slate-300 text-sm font-medium px-2 leading-relaxed">
              Sıfırlama talimatları e-posta adresine gönderildi. Lütfen gelen kutunu (ve gereksiz kutusunu) kontrol et şefim.
            </p>
          </div>
        )}
