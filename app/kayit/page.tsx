    ...
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center p-4 bg-[#050810] relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md relative z-10 bg-[#0b1120] p-8 md:p-10 rounded-3xl border border-white/5 shadow-2xl">
        ...
        <form onSubmit={handleRegister} className="space-y-5">
          ...
          <button type="submit" disabled={isLoading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black uppercase py-4 rounded-xl transition-all shadow-lg shadow-blue-500/20 active:scale-[0.98]">
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
                ...
                <button type="button" onClick={() => handleSocialLogin('Facebook')} className="bg-blue-600/10 border border-blue-600/20 py-3.5 rounded-xl text-[10px] font-black text-blue-500 uppercase hover:bg-blue-600/20 transition-all">Facebook</button>
            </div>
        </div>

        <p className="mt-10 pt-6 border-t border-white/5 text-center text-[11px] font-medium text-slate-500 uppercase tracking-widest">
          Zaten üye misin? <Link href="/giris" className="text-white font-black hover:text-blue-500 ml-2 transition-colors uppercase">Giriş Yap</Link>
        </p>
      </div>
    </div>
  );
