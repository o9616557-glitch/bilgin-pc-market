...
export default function KayitPage() {
  ...
  const handleRegister = (e: React.FormEvent) => {
    e.preventDefault();
    alert("Şefim, kayıt API'sini (Veritabanı bağlantısını) kurduğumuzda bu buton çalışacak.");
  };
    ...
    <div className="min-h-screen bg-[#050814] text-white flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-[#09090b] border border-white/10 rounded-3xl shadow-[0_0_50px_rgba(0,229,255,0.05)] p-8 relative z-10">
        ...
        <div className="flex gap-3 mb-6">
          ...
          <button className="flex-1 bg-white/5 hover:bg-white/10 border border-white/10 py-3 rounded-xl flex items-center justify-center gap-2 transition-all group text-[#1877F2]">
        ...
        <form onSubmit={handleRegister} className="flex flex-col gap-4 mb-6">
          <div className="relative">
            ...
            <input 
              type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Adınız Soyadınız" 
              className="w-full bg-[#050814] border border-white/10 rounded-xl py-3 pl-12 pr-4 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-[#00e5ff]/50 transition-colors" required
            />
          </div>
          <div className="relative">
            <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500" size={18} />
            <input 
              type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="E-Posta Adresiniz" 
              className="w-full bg-[#050814] border border-white/10 rounded-xl py-3 pl-12 pr-4 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-[#00e5ff]/50 transition-colors" required
            />
          </div>
          <div className="relative">
            ...
            <input 
              type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Şifre Belirleyin (En az 6 hane)" 
              className="w-full bg-[#050814] border border-white/10 rounded-xl py-3 pl-12 pr-4 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-[#00e5ff]/50 transition-colors" minLength={6} required
            />
          </div>
          
          <button type="submit" className="w-full bg-[#00e5ff] text-black rounded-xl py-3.5 font-black uppercase tracking-widest hover:bg-[#00c4db] transition-all shadow-[0_0_20px_rgba(0,229,255,0.2)] mt-2 flex items-center justify-center gap-2">
            <ShieldCheck size={18} /> KAYIT OL
          </button>
        </form>
