    ...
    <div className="min-h-screen bg-[#050814] text-white pb-12 pt-8 md:pt-12">
      <div className="ana-konteynir" style={{ maxWidth: "1200px", margin: "0 auto", padding: "20px" }}>
        ...
        <div className="odeme-konteynir" style={{ display: "flex", gap: "30px" }}>
            ...
            <form onSubmit={siparisTamamla} className="bg-[#09090b] border border-slate-800/50 rounded-3xl p-6 sm:p-8">
              ...
              <div className="bg-[#121215] border border-slate-800 p-4 rounded-xl mb-6 text-center">
                <p className="text-slate-400 text-xs sm:text-sm leading-snug">
                  Siparişi onaylayarak sözleşmeleri okuyup kabul etmiş sayılırsınız.
                </p>
              </div>
              ...
              {!iyzicoFormHtml && (
                <button 
                  type="submit" 
                  disabled={yukleniyor} 
                  className={`w-full font-black uppercase tracking-wider py-4 rounded-xl text-lg transition-all ${odemeYontemi === "havale" ? "bg-[#10b981] hover:bg-[#0ea5e9] text-black shadow-[0_0_15px_rgba(16,185,129,0.3)]" : "bg-[#00e5ff] hover:bg-[#00c4db] text-black shadow-[0_0_15px_rgba(0,229,255,0.3)]"}`}
                >
          ...
          {/* ➡️ SAĞ TARAF: SİPARİŞ ÖZETİ (ÖDEME SAYFASI SATIŞ STRATEJİSİ YAN YANA) */}
          <div style={{ flex: "1" }} className="w-full lg:w-[380px] shrink-0">
            <div className="bg-[#09090b] border border-slate-800/50 rounded-3xl p-6 lg:p-8 sticky top-24">
              ...
              {/* 💳 KREDİ KARTI KUTUSU (YAN YANA AKTİF/PASİF) */}
              <div className={`border rounded-2xl p-4 mb-4 transition-all relative overflow-hidden flex items-center justify-between gap-2 ${odemeYontemi === "kart" ? "bg-[#121215] border-[#00e5ff]/50 shadow-[0_0_15px_rgba(0,229,255,0.1)]" : "bg-transparent border-slate-800 opacity-50"}`}>
                <div className="flex items-center gap-2 relative z-10 shrink-0">
                  <CreditCard className={`w-5 h-5 sm:w-6 sm:h-6 ${odemeYontemi === "kart" ? "text-[#00e5ff]" : "text-slate-400"}`} />
                  <span className={`font-black text-sm sm:text-base whitespace-nowrap ${odemeYontemi === "kart" ? "text-white" : "text-slate-400"}`}>Kredi Kartı</span>
                </div>
                <div className={`text-xl sm:text-2xl font-black text-right relative z-10 whitespace-nowrap ${odemeYontemi === "kart" ? "text-[#00e5ff]" : "text-slate-400"}`}>
                  {genelToplam.toLocaleString("tr-TR")} TL
                </div>
              </div>

              {/* 💸 HAVALE KUTUSU (YAN YANA AKTİF/PASİF) */}
              {toplamHavaleIndirimi > 0 && (
                <div className={`border rounded-2xl p-4 transition-all relative overflow-hidden flex items-center justify-between gap-2 ${odemeYontemi === "havale" ? "bg-[#10b981]/10 border-[#10b981]/50 shadow-[0_0_15px_rgba(16,185,129,0.15)]" : "bg-transparent border-slate-800 opacity-50"}`}>
                  {odemeYontemi === "havale" && <div className="absolute top-0 right-0 w-24 h-24 bg-[#10b981] blur-[50px] opacity-20 pointer-events-none"></div>}
                  <div className="flex items-center gap-2 relative z-10 shrink-0">
                    <Banknote className={`w-5 h-5 sm:w-6 sm:h-6 ${odemeYontemi === "havale" ? "text-[#10b981]" : "text-slate-400"}`} />
                    <span className={`font-black text-sm sm:text-base whitespace-nowrap ${odemeYontemi === "havale" ? "text-[#10b981]" : "text-slate-400"}`}>Havale / EFT</span>
                  </div>
                  <div className={`text-xl sm:text-2xl font-black text-right relative z-10 whitespace-nowrap ${odemeYontemi === "havale" ? "text-[#10b981] drop-shadow-[0_0_10px_rgba(16,185,129,0.3)]" : "text-slate-400"}`}>
