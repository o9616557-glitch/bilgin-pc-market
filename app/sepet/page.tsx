...
export default function CartPage() {
  ...
  useEffect(() => {
    setIsMounted(true);
    // Hafızadaki sepeti çekiyoruz (Artık kodun içinde sahte ürün ekleme kısmı YOK)
  ...
  // 👑 SİLME ÖZELLİĞİ: Çöp kutusuna basınca veya miktar 0'a inince ürünü sepetten tamamen kazır
  const updateQuantity = (id: number, type: 'increase' | 'decrease' | 'remove') => {
    ...
    if (type === 'remove') {
      ...
      updated = updated.map(item => {
        ...
        return item;
      }).filter(item => item.quantity > 0); // Adet 0'a düşerse ürünü siler
    }
        ...
        {cart.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-4 bg-[#0B1121] border border-[#1A233A] rounded-xl p-6 space-y-6">
              ...
              {/* O KUSURSUZ ÇALIŞAN MOTORUN (checkout) TETİKLENDİĞİ BUTON */}
              <a 
