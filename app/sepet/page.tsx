...
export default function SepetSayfasi() {
  ...
  const kargo = (araToplam > 5000 || araToplam === 0) ? 0 : 1; // Şefim kargo 150 değil, ödemedeki gibi 1 TL / Ücretsiz ayarına çektim
            ...
            {sepet.map((urun: any, index: number) => {
              ...
              return (
                <div key={index} className="flex flex-col sm:flex-row items-center sm:items-stretch bg-[#09090b] border border-white/5 rounded-2xl p-4 gap-4 transition-all hover:border-[#3b82f6]/30 shadow-sm">
                  ...
                  <div className="w-24 h-24 sm:w-28 sm:h-28 shrink-0 bg-[#121215] rounded-xl border border-white/10 flex items-center justify-center p-2 shadow-inner">
                  ...
                  <div className="flex-1 flex flex-col text-center sm:text-left w-full justify-center">
                    {/* 🚀 BİNGO: Ürün ismi artık mobilde tam gözükür (break-words) */}
                  ...
                  <div className="flex flex-row items-center justify-between sm:flex-col sm:justify-center gap-3 w-full sm:w-auto mt-3 sm:mt-0 pt-4 sm:pt-0 border-t border-white/5 sm:border-none">
                    <div className="flex items-center bg-[#121215] border border-white/10 rounded-xl p-1 shadow-inner">
                      <button onClick={() => adetGuncelle(urun.id, urun.varyasyon, -1)} className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-all font-medium text-lg">-</button>
                      <span className="font-black w-8 text-center text-white text-sm sm:text-base">{urun.adet}</span>
                      <button onClick={() => adetGuncelle(urun.id, urun.varyasyon, 1)} className="w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-all font-medium text-lg">+</button>
                    </div>
                    <button onClick={() => setUrunToDelete(urun)} className="w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center text-slate-400 bg-[#121215] border border-white/10 hover:bg-rose-500/10 hover:border-rose-500/30 hover:text-rose-400 rounded-xl transition-all shrink-0" title="Sepetten Çıkar">
