          ...
          {favoriteProducts.length === 0 ? (
            <div className="bg-[#0f172a] border border-slate-800 rounded-2xl p-10 sm:p-16 flex flex-col items-center justify-center text-center shadow-xl">
              ...
              <p className="text-slate-400 text-sm max-w-sm mx-auto mb-8 font-medium leading-relaxed">
                İlginizi çeken donanımları kalp ikonuna tıklayarak favori listenize ekleyebilirsiniz.
              </p>
              <Link 
                href="/" 
                prefetch={true} 
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
          <div className="bg-[#0f172a] border border-slate-800 rounded-3xl p-6 sm:p-8 max-w-sm w-full flex flex-col items-center text-center shadow-[0_20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="flex w-full gap-3">
              <button onClick={() => setProductToDelete(null)} className="flex-1 bg-[#020617] border border-slate-800 text-slate-400 hover:text-white font-bold py-3.5 rounded-xl text-xs uppercase tracking-wider">İptal</button>
              <button onClick={handleDeleteFavorite} className="flex-1 bg-gradient-to-r from-red-600 to-rose-600 text-white font-bold py-3.5 rounded-xl text-xs uppercase tracking-wider shadow-lg">Evet, Kaldır</button>
            </div>
          </div>
        </div>
      )}
