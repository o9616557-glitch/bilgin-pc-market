...
export async function GET(req: Request) {
  try {
    ...
    // 🚀 BİNGO 2: Saat farkını yenen çözüm (Date.now yerine obje yolluyoruz)
    const suAnkiTarih = new Date();

    const user = await User.findOne({
      pendingDeviceToken: token,
      pendingDeviceExpires: { $gt: suAnkiTarih } 
    });
    ...
    // 1. DURUM: Link zaman aşımına uğramışsa
    if (!user) {
      const html = htmlSayfaUret("BAĞLANTI ZAMAN AŞIMI", "Güvenlik standartlarımız gereği bu onay bağlantısının kullanım süresi dolmuş veya işlem daha önce tamamlanmıştır.<br><br>Lütfen giriş ekranına dönerek işlemi yeniden başlatınız.", "⏳", "#f59e0b");
      return new NextResponse(html, { status: 400, headers: { "Content-Type": "text/html; charset=utf-8" } });
    }

    // 2. DURUM: BAŞARILI ONAY VE İÇERİ ALMA (VIP BİLET BURADA KESİLİYOR)
    if (action === "approve") {
      if (!user.trustedDevices.includes(user.pendingDeviceInfo.cihaz)) {
        user.trustedDevices.push(user.pendingDeviceInfo.cihaz);
      }
      
      // Mongoose bu satırı User.ts'deki 'karantinaPass' alanı sayesinde artık başarıyla kaydedecek!
      ...
      const kurumsalMesaj = `
        ...
        <strong style="color:#10b981;">Tekrar giriş yaptığınızda sistem sizi otomatik olarak içeri alacaktır.</strong>
      `;
      const html = htmlSayfaUret("CİHAZ YETKİLENDİRİLDİ", kurumsalMesaj, "🛡️", "#3b82f6");
      return new NextResponse(html, { status: 200, headers: { "Content-Type": "text/html; charset=utf-8" } });
    } 
    
    // 3. DURUM: REDDETME EKRANI
      ...
      user.pendingDeviceInfo = undefined;
      await user.save();
      const html = htmlSayfaUret("ERİŞİM ENGELLENDİ", "Talebiniz üzerine bu giriş işlemi sistem tarafından <b>derhal iptal edilmiş</b> ve hesabınız koruma altına alınmıştır.", "🚨", "#ef4444");
      return new NextResponse(html, { status: 200, headers: { "Content-Type": "text/html; charset=utf-8" } });
    }
  } catch (error) {
