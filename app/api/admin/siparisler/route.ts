...
export async function PUT(request: Request) {
  try {
    ...
    if (yeniDurum !== undefined) {
      // 🚀 BİNGO: İki tarafı da eşitliyoruz. Müşteri kodu nereye bakarsa baksın doğruyu görecek!
      guncellenecekler.durum = yeniDurum;
      guncellenecekler.status = yeniDurum; 
    }
    if (musteriMesaji !== undefined) {
      guncellenecekler.musteriMesaji = musteriMesaji;
    }
    ...
    if (siparis && yeniDurum && (yeniDurum === "Ödendi / Hazırlanıyor" || yeniDurum === "Kargoya Verildi" || yeniDurum === "İptal Edildi")) {
      ...
      if (musteriMaili) {
        ...
        try {
          await transporter.sendMail(mailSecenekleri);
          console.log("İptal/Onay E-postası Başarıyla Gönderildi!");
        } catch (mailHatasi) {
          console.error("E-posta gönderim hatası:", mailHatasi);
        }
      }
    }
    ...
    console.error("Durum güncellerken hata:", error);
