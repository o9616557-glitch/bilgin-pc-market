    ...
    <div className="min-h-[calc(100vh-80px)] bg-[#050810] text-white py-10 px-4 font-sans relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto bg-[#0b1120] border border-white/5 rounded-3xl shadow-2xl p-6 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-10 relative z-10">
        <div className="relative aspect-square w-full bg-[#050810] border border-white/5 rounded-2xl p-6 flex items-center justify-center group overflow-hidden">
          <button onClick={handleToggleFavorite} className={`absolute top-4 right-4 z-20 w-10 h-10 rounded-xl border flex items-center justify-center transition-all shadow-lg ${isFavorite ? "bg-blue-600 border-blue-500 text-white shadow-[0_0_15px_#3b82f6]" : "bg-[#050810]/80 border-white/5 text-slate-400 hover:text-white"}`}>
            <svg className="w-5 h-5" fill={isFavorite ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
            </svg>
          </button>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={product.images?.[0]?.src || "https://via.placeholder.com/500"} alt={product.name} className="w-full h-full object-contain" />
        </div>
        ...
        <div className="flex flex-col justify-between space-y-6">
          <div className="space-y-4">
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="text-xs text-slate-400 leading-relaxed max-h-[180px] overflow-y-auto pr-2 border-t border-white/5 pt-2" dangerouslySetInnerHTML={{ __html: product.description || product.short_description || "Teknik detay bulunmamaktadır." }} />
            </div>
          </div>

          <div className="pt-4 border-t border-white/5">
            <button onClick={handleAddToCart} disabled={!product.in_stock} className="w-full bg-blue-600 hover:bg-blue-500 disabled:bg-white/5 disabled:text-slate-600 text-white font-black uppercase tracking-widest text-xs py-4 rounded-xl transition-all flex items-center justify-center gap-3">
